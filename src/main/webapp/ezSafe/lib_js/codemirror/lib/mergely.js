...
//2015-5-13 Add By liwei  Comment: fixed ie8 bind method issue
...
//  2015-5-13 modified by liwei Comment: fix ie8 Object.keys issues
...
jQuery.extend(Mgly.CodeMirrorDiffView.prototype, {
	...
	bind: function(el) {
		...
		if (jQuery.button != undefined) {
			//jquery ui
			merge_lhs_button = '<button title="Merge 稳定版本"></button>';
			merge_rhs_button = '<button title="Merge 当前版本"></button>';
		}
		else {
			// homebrew
			...
			merge_lhs_button = '<div style="' + style + '" title="Merge 稳定版本">&lt;</div>';
			merge_rhs_button = '<div style="' + style + '" title="Merge 当前版本">&gt;</div>';
		}
		...
		var rmargin;
		if (bowser.msie && bowser.version < 10 ) //modify By liwei 2015/5/13 增加对IE10以内的浏览器模拟Html5 canvas 功能支持
		{
			//<canvas id="' + this.id + '-lhs-margin" width="8px" height="' + height + '"></canvas>
			var lhs_tmp = document.createElement('canvas');
			this.element.append(jQuery('<div class="mergely-margin" style="height: ' + height + '"></div>').append(lhs_tmp));
			G_vmlCanvasManager.initElement(lhs_tmp);
			$(lhs_tmp).width(8).height(height).attr("id",this.id+"-lhs-margin");
			...
			//<canvas id="' + this.id + '-lhs-' + this.id + '-rhs-canvas" style="width:28px" width="28px" height="' + height + '"></canvas>
			var rhs_temp = document.createElement('canvas');
			this.element.append(jQuery('<div class="mergely-canvas" style="height: ' + height + '"></div>').append(rhs_temp));
			G_vmlCanvasManager.initElement(rhs_temp);
			$(rhs_temp).width(28).height(height).attr("id",this.id+ '-lhs-' + this.id + '-rhs-canvas');

			//<canvas id="' + this.id + '-rhs-margin" width="8px" height="' + height + '"></canvas>
			var margin_tmp = document.createElement('canvas');
			 rmargin = jQuery('<div class="mergely-margin" style="height: ' + height + '"></div>').append(margin_tmp);
			G_vmlCanvasManager.initElement(margin_tmp);
			$(margin_tmp).width(8).height(height).attr("id",this.id+ '-rhs-margin');
		}
		else
		{
			...
			this.element.append(jQuery('<div class="mergely-canvas" style="height: ' + height + '"><canvas id="' + this.id + '-lhs-' + this.id + '-rhs-canvas" style="width:28px" width="28px" height="' + height + '"></canvas></div>'));
			 rmargin = jQuery('<div class="mergely-margin" style="height: ' + height + '"><canvas id="' + this.id + '-rhs-margin" width="8px" height="' + height + '"></canvas></div>');
		}
