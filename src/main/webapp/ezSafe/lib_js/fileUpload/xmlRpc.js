...
XmlRpcRequest.prototype.setHeader = function (a, b) {
    if (b) {
        this.headers[a] = b
    } else {
...
XmlRpcRequest.prototype.send = function (options) {
    var c = "", a = 0, b, d;
    ...
    d = (typeof window.XMLHttpRequest != "undefined") ? new XMLHttpRequest() : new ActiveXObject("Microsoft.XMLHTTP");
    d.open("post", this.serviceUrl);
    //for (a in this.headers) {
    //    if (this.headers.hasOwnProperty(a)) {
    //        d.setRequestHeader(a, this.headers[a])
    //    }
    //}
    //if (this.withCredentials && "withCredentials" in d) {
    //    d.withCredentials = true
    //}
    d.onreadystatechange = function () {
        if (d.readyState == 4 && d.status == 200) {
            if (options && typeof options.success == "function") {
                var response = new XmlRpcResponse(Builder.buildDOM(d.responseText));
                var result = null;
                if (response) {
                    result = eval('(' + response.parseXML() + ')');
                }
                options.success(result);
            }
            d = null;
        //} else if (d.status == 12029 || d.statusText === "Unknown"){
        //    if (options && typeof options.error == "function") {
        //        options.error("上传失败，请确认上传客户端是否已开启！");
        //    }
        //}
        } else {
            //if (options && typeof options.error == "function") {
...
Builder.buildXHR = function (a) {
    if (a) {
        return (typeof window.XDomainRequest != "undefined") ? new XDomainRequest() : new XMLHttpRequest()
    } else {
